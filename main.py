...
async def post_init(application: Application):
    ...
    if application.job_queue:
        # Run cleanup every 5 minutes, starting after 10 seconds
        application.job_queue.run_repeating(
            cleanup_overdue_messages,
            interval=300,
            first=10
        )
        log.info("🟢 Periodic message cleanup scheduled")
    else:
        log.warning("⚠️ Job queue not available - auto-delete feature will not work")
...
def start_bot():
    ...
    # Build application with post_init to set up job queue after initialization
